...
def SimpleRNN(embedding_vector_length, max_review_length):
    ...
    # 2만여개 중 1000개의 단어 사용, embedding_vector_length=32(32개의 차원으로 축소(원-핫 인코딩)=단어가 해당하는 곳만 1로 표시)
    # 임베딩은 차원을 축소하는 개념이라고 생각하면 된다, output_dim: 결과로 나올 임베딩 벡터의 크기(차원)
    # input_length: 들어오는 단어 벡터의 크기(300)
    model.add(tf.keras.layers.Embedding(input_dim = 1000, output_dim = embedding_vector_length, input_length = max_review_length))
    model.add(tf.keras.layers.SimpleRNN(3)) # GPU가 아니면 자연어는 학습이 거의 안된다. 너무 오래 걸린다.
...
def main():
    ...
    model_simple_rnn_history = model_simple_rnn.fit(X_train, y_train, epochs = 2, batch_size = 256, verbose = 2)
    print('\n')
    model_lstm_history = model_lstm.fit(X_train, y_train, epochs = 2, batch_size = 256, verbose = 2)
    print('\n')
    model_gru_history = model_gru.fit(X_train, y_train, epochs = 2, batch_size = 256, verbose = 2)

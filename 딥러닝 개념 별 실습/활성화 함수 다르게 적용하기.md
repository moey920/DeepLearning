...
def main():
    ...
    model_elu.compile(loss = 'sparse_categorical_crossentropy', optimizer = 'adam', metrics = ['accuracy'])
    
    # model_sig.summary()
    # model_relu.summary()
    # model_tanh.summary()
    
    model_sig_history = model_sig.fit(x_train, y_train, epochs=10, batch_size=128, verbose=0)
    print('\n')
    model_relu_history = model_relu.fit(x_train, y_train, epochs=10, batch_size=128, verbose=0)
    print('\n')
    model_tanh_history = model_tanh.fit(x_train, y_train, epochs=10, batch_size=128, verbose=0)
    model_elu_history = model_elu.fit(x_train, y_train, epochs=10, batch_size=128, verbose=0)
    ...
    scores_elu = model_elu.evaluate(x_test, y_test, verbose=0)
    
    print('\naccuracy_sig: ', scores_sig[-1])
    print('accuracy_relu: ', scores_relu[-1])
    print('accuracy_tanh: ', scores_tanh[-1])
    print('accuracy_elu: ', scores_elu[-1])
    
    return model_sig_history, model_relu_history, model_tanh_history, model_elu_history
